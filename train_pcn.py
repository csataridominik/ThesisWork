...
def save_obj_file2(filename, vertices):
	o3d.io.write_point_cloud(filename, vertices)

# --------------------------------------- Shapenet reading in the emsh, than sampleing from the given planes ----------------------------------------
def load_data_shapenet(train, use_normals,save=False, random = True):
	...
	for folder in tqdm(glob.glob(os.path.join('..','data',curr_folder, '*'))):
		...
		if os.path.exists(model_path):
			...
			if save:
				filename = os.path.join(folder,'models', 'model_normalized_16k_2024_september_30.obj')
	...
	for folder in tqdm(glob.glob(os.path.join('..','data',curr_folder, '*'))):
		...
		if os.path.exists(model_path):
			...
			if random:
				...
				if save:
					filename = os.path.join(folder,'models', 'model_normalized_down_sampled_12k.obj')
					save_obj_file(filename,data)
			else:
				all_data.append([data[:x]])
...
def train(args, model, train_loader, test_loader, boardio, textio, checkpoint):
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if test_loss<best_test_loss:
			...
			torch.save(snap, 'checkpoints/%s/models/best_model_snap_16k_2024_september_30.t7' % (args.exp_name))
			torch.save(model.state_dict(), 'checkpoints/%s/models/best_model_16k_2024_september_30.t7' % (args.exp_name))

		torch.save(snap, 'checkpoints/%s/models/model_snap2.t7' % (args.exp_name))
		torch.save(model.state_dict(), 'checkpoints/%s/models/model_16k_2024_september_30.t7' % (args.exp_name))
