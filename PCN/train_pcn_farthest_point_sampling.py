...
#------------------------------------ ShapeNet Dataset -----------------------------------------------------
def load_data_shapenet8(train, use_normals,save=False, random = False):
	x = 12000
	counter = 0
	...
	all_data = []
	all_label = []
	curr_folder = '..\XYZ_projections_sparse'
...
def load_data_shapenet(train, use_normals,save=False, random = False):
	...
	projection_dir = '..\data\XYZ_projections_sparse\XYZ_'
	...
	for i in tqdm(range(0,100)):
		...
		if coordinates.shape[0]>=1024:
			coordinates_idx = fpsample.bucket_fps_kdline_sampling(coordinates, 1024, h=9)
			coordinates = coordinates[coordinates_idx]
			all_data.append(coordinates)
			all_label.append([label])

	return np.stack(all_data, axis=0), np.stack(all_data, axis=0)
...
def test_one_epoch(device, model, test_loader):
	...
	for i, data in enumerate(tqdm(test_loader)):
		...
		display_original(output['coarse_output'][0].detach().cpu().numpy())
...
def train(args, model, train_loader, test_loader, boardio, textio, checkpoint):
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		if test_loss<best_test_loss:
			...
			torch.save(snap, 'checkpoints/%s/models/THISISMYLASTTRY2.t7' % (args.exp_name))
			torch.save(model.state_dict(), 'checkpoints/%s/models/THISISMYLASTTRY2.t7' % (args.exp_name))
	...
	plt.plot(traini, label='Train Loss')
	plt.plot(testi, label='Test Loss')
	plt.xlabel('Epochs')
	plt.ylabel('Loss')
	plt.legend()
...
def options():
	...
	parser.add_argument('--pretrained', default='THISISMYLASTTRY2.t7', type=str,
...
def main():
	...
	train_size = int(0.9 * len(CommonSet)) 
	...
	model = PCN(emb_dims=1024, num_coarse=numofpoints, detailed_output=False)

	checkpoint = None

	'''if args.pretrained:
		assert os.path.isfile(args.pretrained)
		print("It passedtrhu here...")'''
	model.load_state_dict(torch.load('checkpoints/exp_pcn/models/THISISMYLASTTRY2.t7', map_location='cpu'))
	model.to(args.device)
	
	
	test(args, model, train_loader, textio)
